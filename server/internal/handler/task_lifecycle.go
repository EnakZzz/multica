...
type RerunIssueRequest struct {
	...
	// rather than the issue's current assignee — so clicking retry on a row
	// that belonged to a now-displaced
...
// targets the issue's current agent assignee; if the request body carries
// task_id, the rerun targets the agent that ran that specific past task
// instead. The new task is flagged force_fresh_session=true:

...
const createBuiltInSkill = `-- name: CreateBuiltInSkill :one
INSERT INTO skill (
    workspace_id, name, description, content, config, created_by, is_builtin, builtin_key
) VALUES (
    $1, $2, $3, $4, $5, $6, TRUE, $7
) RETURNING id, workspace_id, name, description, content, config, created_by, created_at, updated_at, is_builtin, builtin_key
`

type CreateBuiltInSkillParams struct {
...
func (q *Queries) CreateBuiltInSkill(ctx context.Context, arg CreateBuiltInSkillParams) (Skill, error) {
	row := q.db.QueryRow(ctx, createBuiltInSkill,
		arg.WorkspaceID,
		arg.Name,
		arg.Description,
		arg.Content,
...
const getBuiltInSkillByKey = `-- name: GetBuiltInSkillByKey :one
SELECT id, workspace_id, name, description, content, config, created_by, created_at, updated_at, is_builtin, builtin_key FROM skill
WHERE workspace_id = $1 AND builtin_key = $2 AND is_builtin = TRUE
`

type GetBuiltInSkillByKeyParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	BuiltinKey  pgtype.Text `json:"builtin_key"`
}

func (q *Queries) GetBuiltInSkillByKey(ctx context.Context, arg GetBuiltInSkillByKeyParams) (Skill, error) {
	row := q.db.QueryRow(ctx, getBuiltInSkillByKey, arg.WorkspaceID, arg.BuiltinKey)
	var i Skill
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
SELECT s.id, s.workspace_id, s.name, s.description, s.content, s.config, s.created_by, s.created_at, s.updated_at, s.is_builtin, s.builtin_key FROM skill s
JOIN agent_skill ask ON ask.skill_id = s.id
WHERE ask.agent_id = $1
ORDER BY s.name ASC
`
...
const updateBuiltInSkill = `-- name: UpdateBuiltInSkill :one
UPDATE skill SET
    name = $2,
    description = $3,
    content = $4,
...
type UpdateBuiltInSkillParams struct {
	ID          pgtype.UUID `json:"id"`
	Name        string      `json:"name"`
	Description string      `json:"description"`
	Content     string      `json:"content"`
...
func (q *Queries) UpdateBuiltInSkill(ctx context.Context, arg UpdateBuiltInSkillParams) (Skill, error) {
	row := q.db.QueryRow(ctx, updateBuiltInSkill,
		arg.ID,
		arg.Name,
		arg.Description,
		arg.Content,

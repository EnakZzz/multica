...
VALUES ($1, $2, NULL, 'queued', $3, $4, COALESCE($5::boolean, FALSE))
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session
`
...
func (q *Queries) GetInternalPlannerAgent(ctx context.Context, workspaceID pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, getInternalPlannerAgent, workspaceID)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
func (q *Queries) UpsertInternalPlannerAgent(ctx context.Context, arg UpsertInternalPlannerAgentParams) (Agent, error) {
	row := q.db.QueryRow(ctx, upsertInternalPlannerAgent,
		arg.WorkspaceID,
		arg.Description,
		arg.RuntimeMode,
		arg.RuntimeID,

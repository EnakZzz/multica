...
SET node_type = 'issue'
WHERE node_type = 'subagent-driven-development';
...
ALTER TABLE pipeline_stage
    ...
    CHECK (node_type IN ('issue', 'manual', 'check', 'spec_review', 'code_review', 'merge'));
...
ALTER TABLE plan_item
    ...
    CHECK (node_type = ANY (ARRAY['issue'::text, 'manual'::text, 'check'::text, 'spec_review'::text, 'code_review'::text, 'merge'::text]));

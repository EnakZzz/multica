...
import { planKeys } from "../plans/queries";
...
function invalidateWorkspaceScopedQueries(qc: QueryClient): void {
  ...
  if (wsId) {
    ...
    qc.invalidateQueries({ queryKey: projectVisualKeys.all(wsId) });
...
export function useRealtimeSync(
  ...
  useEffect(() => {
    ...
    const refreshMap: Record<string, () => void> = {
      ...
      project_visual: () => {
        const wsId = getCurrentWsId();
        if (wsId) qc.invalidateQueries({ queryKey: projectVisualKeys.all(wsId) });
      },
      label: () => {
      ...
      task: () => {
        ...
        qc.invalidateQueries({ queryKey: planKeys.all(wsId) });
      },
    };

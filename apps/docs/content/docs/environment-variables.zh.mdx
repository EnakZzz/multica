...
## Project Wiki Embeddings（可选）

Project Wiki RAG 始终保留关键词检索和 canonical Wiki fallback。只有需要 Wiki / project memory 的语义向量检索时，才需要配置这些变量。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_EMBEDDINGS_API_KEY` | 空 | OpenAI-compatible embeddings endpoint 的 API key。为空时，向量检索和 embedding worker 关闭 |
| `MULTICA_EMBEDDINGS_BASE_URL` | `https://api.openai.com/v1` | embeddings API base URL |
| `MULTICA_EMBEDDINGS_MODEL` | `text-embedding-3-small` | 写入向量记录的 embedding model 名称 |
| `MULTICA_EMBEDDINGS_DIMENSIONS` | `1536` | Multica pgvector 表当前期望的向量维度 |

Wiki / memory 保存事务提交后只会入队 embedding job；失败会在后台重试，不阻塞 Wiki 保存或 Plan 生成。已有 Wiki 不会在启动时自动全量 backfill；请显式运行 `multica project wiki backfill-embeddings <project-id>` 或调用 backfill API。
...
当 checkout 的 `origin` remote 指向 GitLab 时，`multica repo integrate --strategy pr-first` 可以为本地 agent 的合入任务创建或复用 GitLab Merge Request。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `GITLAB_BASE_URL` | 从 `origin` 推断 | 自建 GitLab 的基础 URL，例如 `https://gitlab.example.com` |
| `GITLAB_TOKEN` | 空 | 作为 `PRIVATE-TOKEN` 使用的 personal/project/group access token；也支持 `GLAB_TOKEN` 和 `GITLAB_PRIVATE_TOKEN` |
| `CI_JOB_TOKEN` | 空 | 在 GitLab CI 中作为 `JOB-TOKEN` 使用 |

如果没有配置 GitLab token，PR-first 会返回 failed JSON，不会自动降级成 direct merge。

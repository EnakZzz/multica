...
## Project Wiki embeddings (optional)

Project Wiki RAG always keeps keyword search and canonical Wiki fallback available. Set these variables only when you want semantic vector retrieval for Wiki and project memory.

| Variable | Default | Description |
|---|---|---|
| `MULTICA_EMBEDDINGS_API_KEY` | empty | API key for an OpenAI-compatible embeddings endpoint. When empty, vector retrieval and embedding workers are disabled |
| `MULTICA_EMBEDDINGS_BASE_URL` | `https://api.openai.com/v1` | Base URL for the embeddings API |
| `MULTICA_EMBEDDINGS_MODEL` | `text-embedding-3-small` | Embedding model name stored with each vector |
| `MULTICA_EMBEDDINGS_DIMENSIONS` | `1536` | Vector dimension expected by Multica's pgvector table |

Wiki and memory edits enqueue embedding jobs after the save transaction commits. Failures are retried in the background and never block Wiki saves or Plan generation. Existing Wiki pages are not backfilled automatically on startup; run `multica project wiki backfill-embeddings <project-id>` or call the backfill API explicitly.
...
`multica repo integrate --strategy pr-first` can create or reuse GitLab Merge Requests for local agent merge tasks when the checkout's `origin` remote points at GitLab.

| Variable | Default | Description |
|---|---|---|
| `GITLAB_BASE_URL` | inferred from `origin` | Base URL for self-managed GitLab, for example `https://gitlab.example.com` |
| `GITLAB_TOKEN` | empty | Personal/project/group access token used as `PRIVATE-TOKEN`; `GLAB_TOKEN` and `GITLAB_PRIVATE_TOKEN` are also accepted |
| `CI_JOB_TOKEN` | empty | Used as `JOB-TOKEN` when running inside GitLab CI |

If no GitLab token is configured, PR-first returns a failed JSON result and does not fall back to a direct merge.
